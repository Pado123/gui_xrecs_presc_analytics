...
class ExperimentManager:
    ...
    def __init__(self, data_path: Path, output_path: Path, output_filename: str, model: GenAI, 
        ...
        print(f"ExperimentManager initialized with data_path={data_path}, output_path={output_path}, output_filename={output_filename}")

    @staticmethod
    def load_data(data_path: Path, n_samples: int, train_seed: int, test_seed: int, mode: str = "agg") -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        Args:
            ...
            mode: Data loading mode ("agg" or "seq")
        
        Returns:
            tuple of training and test DataFrames
        """
        # Determine file suffix based on mode
        file_suffix = "aggr_hist" if mode == "agg" else "sequential"
        
        if mode == "agg":
            columns_to_drop = ["time:timestamp", "case:concept:name", 
                            "day_of_week", "hour_of_day"]
            
            train = pd.read_csv(data_path / f"preprocessed_log_{file_suffix}_train.csv").drop(columns=columns_to_drop)
            test = pd.read_csv(data_path / f"preprocessed_log_{file_suffix}_test.csv").drop(columns=columns_to_drop)
        else:  # seq mode
        ...
        test = test.sample(frac=1, random_state=test_seed).reset_index(drop=True).head(20)
        # print("ATTENTION ONLY 10 SAMPLES")
        
        return train, test

    def evaluate_predictions(self, y_true: List[float], y_pred: List[float]) -> tuple[float, float, float]:
    ...
    def run(self, sample_sizes: List[int] = [2, 10, 50, 100, 500]):
        ...
        for n_samples in sample_sizes:
            ...
            for test_idx in range(self.n_seeds):
                ...
                _, test_data = self.load_data(self.data_path, n_samples, test_seed, test_seed, mode=self.mode)
                ...
                for train_idx in range(self.n_seeds):
                    ...
                    train_data, _ = self.load_data(self.data_path, n_samples, train_seed, test_seed, mode=self.mode)
...
def main():
    ...
    assert not log_file.exists(), f"Log file already exists: {log_file}"
    ...
    experiment = ExperimentManager(data_path, output_path, output_filename, model, n_seeds=5)
